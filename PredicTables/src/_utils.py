...
output_map = {'dataframe': 'dataframe',
              'df': 'dataframe',
              'data': 'dataframe',
              'table': 'dataframe',
              'series': 'series',
...
def _to_numpy(data: Union[pd.DataFrame,
                          pd.Series,
                          pl.DataFrame,
                          pl.Series,
                          pl.LazyFrame,
    ...
    if isinstance(data, pd.DataFrame):
        ...
        raise TypeError(f"Input data type {type(data)} not supported. \n\
...
def _to_polars(data: Union[pd.DataFrame,
                           pd.Series,
                           pl.DataFrame,
                           pl.Series,
                           pl.LazyFrame,
                           np.ndarray,
                           list],
               to:str = 'dataframe') -> Union[pl.DataFrame, pl.Series, pl.LazyFrame]:
    """
    ...
    if isinstance(data, pd.DataFrame):
        if to == 'dataframe':
            ...
            raise ValueError(f"Output type {to} not supported for input type {type(data)}.")
    elif isinstance(data, pd.Series):
        if to == 'dataframe':
...
def _to_pandas(data: Union[pd.DataFrame,
                           pd.Series,
                           pl.DataFrame,
                           pl.Series,
                           pl.LazyFrame,
                           np.ndarray,
                           list],
               to:str = 'dataframe') -> Union[pd.DataFrame, pd.Series]:
    """
    ...
    if isinstance(data, pd.DataFrame):
        if to == 'dataframe':
            ...
            raise ValueError(f"Cannot convert to pandas lazy frame. \
There is no such thing as a lazy pandas frame.")
        ...
        if to == 'dataframe':
            ...
            raise ValueError("Cannot convert to pandas lazy frame. \
There is no such thing as a lazy pandas frame.")
        else:
            raise ValueError(f"Output type {to} not supported for input type {type(data)}.")
    elif isinstance(data, pl.DataFrame):
...
def _select_binary_columns(data: Union[pd.DataFrame,
                                       pl.DataFrame,
                                       pl.LazyFrame],
                           missing_col:str = "999") -> list:
    """
    ...
    binary = categorical.select(
        [col for col in categorical.columns if categorical.select([col]).unique().collect().shape[0] <= 3]
    )

    # Drop any other columns that have something besides "0", "1", and missing_col as categories
    binary = binary.select(
        [col for col in binary.columns if set(binary.select([col]).unique().collect().select('gt_mean').unique().to_series()) <= {'0', '1', missing_col}]
    )
