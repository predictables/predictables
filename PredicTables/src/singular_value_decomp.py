...
class SingularValueDecomposition:
    ...
    def subspace_iteration(self):
        ...
        # Initial values
        A = self.A
        V = pl.DataFrame(pl.Series([1] + ([0] * (self.n_rows - 1))).cast(pl.Float32).alias('Col0'))

        V = V.with_columns([
            pl.Series(([0] * i) + [1] + ([0] * (self.n_rows - i - 1))).cast(pl.Float32).alias(f"Col{i}") for i in range(1, self.n_rows)
        ])
