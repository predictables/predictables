...
def calc_continuous_categorical_corr_df(
    ...
    if method not in ["spearman", "anova", "ancova"]:
        raise ValueError(
            f"method must be one of the following: 'spearman', 'anova', 'ancova', but got {method}"
        )
...
def anova_correlation_coef_df(
    ...
    when assessing the relationship between a continuous variable and a categorical variable.
...
def anova_correlation_coef_series(
    ...
    if s1_type == "continuous" and s2_type == "categorical":
        ...
        raise TypeError(
            f"Expected one continuous and one categorical variable, but got {s1_type} and {s2_type}"
        )
    ...
    # Using eta2 as a measure of correlation strength
    return eta_sq
...
def ancova_correlation_coef_df(
    ...
    for cont_col in continuous_vars:
        for cat_col in categorical_vars:
            # ANCOVA model formula - the covariates are just the other levels of the categorical variable
...
def spearmans_rank_correlation_coef_df(
    ...
    preserves the order (if any) of the categories is essential. Spearman's rho should be
    interpreted with the nature of the categorical variable in mind, as it reflects the
    strength of a monotonic, not necessarily linear, relationship.
    ...
    # Calculate rank-based correlations for each pair of continuous and categorical variables
    for cont_col in continuous_vars:
        for cat_col in categorical_vars:
...
def spearmans_rank_correlation_coef_series(
    ...
    # Calculate rank-based correlation differently depending on whether the categorical variable
    # is binary or multi-level
    if s2.nunique() == 2:
        # Spearman's rank correlation for binary categorical variables
