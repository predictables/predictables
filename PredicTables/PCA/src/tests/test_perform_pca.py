...
from PredicTables.util import to_pl_lf, to_pd_df, to_pl_df
...
def test_pca_data_formats(mock_data, data_format):
    ...
    # Validate the result
    assert (
        pca_result.components_.shape == (n_components, X_train_formatted.shape[1])
...
def test_pca_high_dimensional_data():
    ...
    # Validate the result
    assert (
        pca_result.components_.shape == (n_components, 100)
