...
from Univariates._feature_engineering_params import NON_NUMERIC_COLUMNS, \
                                                    NUMERIC_CATEGORICAL_COLUMNS, \
                                                    NON_CATEGORICAL_COLUMNS

## functions used below:
def aggregate_features(df:Union[pl.LazyFrame, pl.DataFrame],
                        num_cols:List[str],
                        ...
                        include_relativities:bool = False) -> pl.LazyFrame:
    """
    ...
    for gb_col in groupby_cols:
        for num_col in num_cols:
            temp_agg = df.select([pl.col(gb_col).fill_null('none'), pl.col(num_col).drop_nans().drop_nulls()])\
                            .groupby(gb_col)\
                            .agg(func(pl.col(num_col))\
    ...
    for i, agg in enumerate(aggs):
        ...
        if i==0:
            out_df = df.join(agg.select(agg.columns).with_columns(pl.col(agg.columns[1]).fill_nan(0)),
                            on=agg.columns[0], how="left")
        else:
            out_df = out_df.join(agg.select(agg.columns).with_columns(pl.col(agg.columns[1]).fill_nan(0)),
                                on=agg.columns[0], how="left")
    ...
    if agg_func is None:
        ...
        if isinstance(agg_func, list):
            for i, f in enumerate(agg_func):
                out_df = out_df.with_columns(f(pl.col(agg_cols[i]), pl.col(agg_cols[i+1])).alias(f"{agg_func_name[i]}_{[agg_cols[i]]}({[agg_cols[i+1]]})").cast(pl.Float64).fill_null(0))
        else:
            out_df = out_df.with_columns(agg_func(pl.col(agg_cols)).alias(f"{agg_func_name}_{agg_cols}").cast(pl.Float64).fill_null(0))
    ...
    if include_relativities:
        for num, cat in product(num_cols, groupby_cols):
            ...
            out_df = out_df.with_columns([pl.col(num).alias(f"[{num}] : [{cat}].{func_name}").fill_nan(0) / pl.col(colname) ])
        
    return out_df

def aggregate_ratios(df:Union[pl.LazyFrame, pl.DataFrame],
                     num_cols:List[str],
                     groupby_cols:List[str],
                     include_ratios:bool = True) -> pl.LazyFrame:
    """
    ...
    aggs = []
    for gb_col in groupby_cols:
        temp_agg = df.select([pl.col(gb_col).fill_null('none'), 
                              pl.col(num_cols[0]).drop_nans().drop_nulls(), 
                              pl.col(num_cols[1]).drop_nans().drop_nulls()])\
                    .groupby(gb_col)\
                    .agg(pl.col(num_cols[0]).sum() / pl.col(num_cols[1]).sum()\
                            .alias(f"avg_ratio_{gb_col}").cast(pl.Float64).fill_null(0))
    ...
    for i, agg in enumerate(aggs):
        if i==0:
            out_df = df.join(agg.select(agg.columns).with_columns(pl.col(agg.columns[1]).fill_nan(0)), on=agg.columns[0], how="left")
        else:
            out_df = out_df.join(agg.select(agg.columns).with_columns(pl.col(agg.columns[1]).fill_nan(0)), on=agg.columns[0], how="left")
...
transformations = namedtuple("transformations", ["log",
                                                 "inv",
                                                 "sqrt",
                                                 "cbrt",
                                                 "polynomial2",
...
class FeatureEngineering:
    def __init__(self,
        ...
        self.transformations = transformations(log = False,
                                               inv = False,
                                               sqrt = False,
                                               cbrt = False,
                                               polynomial2 = False,
        ...
        if isinstance(self.df, pd.DataFrame):
            self.df = pl.from_pandas(self.df.convert_dtypes(dtype_backend="pyarrow"))\
                        .lazy()
        ...
        self.cat_cols = [col for col in self.df.columns \
        ...
        self.cat_cols = self.cat_cols + [col for col in NUMERIC_CATEGORICAL_COLUMNS() \
                                            if col not in self.cat_cols]

        # remove any categorical columns that are in the NON_CATEGORICAL_COLUMNS list
        self.cat_cols = [col for col in self.cat_cols \
        ...
        self.num_cols = [col for col in self.num_cols \
    ...
    def AddLog(self, return_:bool = False):
        ...
        if self.transformations.log:
            ...
            cat_cols = [field for field, dtype in self.df.schema.items() \
                                                if dtype==pl.Categorical or \
                                                    dtype==pl.Object or \
                                                    dtype==pl.Utf8]
            ...
            log_df = log_df.with_columns([pl.col(col)\
                                            .log()\
                                            .alias(f"log({col})") \
            ...
            self.log_df.collect().write_parquet(f"{self.output_data_path}log_df.parquet")
    ...
    def AddInverse(self, return_:bool = False):
        ...
        if self.transformations.inv:
            ...
            inv_df = inv_df.with_columns([pl.col(col)\
                                            .pow(-1)\
                                            .alias(f"1/{col}") \
            ...
            self.inv_df.collect().write_parquet(f"{self.output_data_path}inv_df.parquet")
    ...
    def AddSquareRoot(self, return_:bool = False):
        if self.transformations.sqrt:
            ...
            sqrt_df = sqrt_df.with_columns([pl.col(col).sqrt() \
                                            .alias(f"sqrt({col})") \
            ...
            self.sqrt_df.collect().write_parquet(f"{self.output_data_path}sqrt_df.parquet")
    ...
    def AddCubeRoot(self, return_:bool = False):
        if self.transformations.cbrt:
            ...
            #                             .fill_null(0) + 1 \
            #                             for col in self.num_cols])
            cbrt_df = cbrt_df.with_columns([pl.col(col).cbrt()\
                                            .alias(f"cbrt({col})")\
                                            for col in cbrt_df.columns])
            ...
            self.cbrt_df.collect().write_parquet(f"{self.output_data_path}cbrt_df.parquet")
    ...
    def AddPolynomial2(self, return_:bool = False):
        ...
        if self.transformations.polynomial2:
            ...
            for i, col in enumerate(self.df.columns):
                if (col not in cat_cols) and \
                   (~pd.Series(self.df.columns).str.lower().str.contains('dat')).tolist() and \
                   (~pd.Series(self.df.columns).str.lower().str.contains('_dt')).tolist() and \
                   (~pd.Series(self.df.columns).str.lower().str.contains('_id')).tolist() and \
                   (~pd.Series(self.df.columns).str.lower().str.contains('_ind')).tolist() and \
                   (~pd.Series(self.df.columns).str.lower().str.contains('missing')).tolist() and \
                    (self.df.schema[col] != pl.Date) and \
                    (self.df.schema[col] != pl.Datetime) and \
                    (self.df.schema[col] != pl.Boolean) and \
                    (self.df.schema[col] != pl.Categorical) and \
                    (self.df.schema[col] != pl.Object) and \
                    (self.df.schema[col] != pl.Utf8):
                    col_list.append((pl.col(col).cast(pl.Float32).fill_null(-1) + 2).alias(col))
                else:
                    pass

            poly2_select = []
            cols = [col for col in self.df.select(col_list).columns if col not in cat_cols]
            for col1, col2 in tqdm(product(cols, cols),
                                    total=len(cols)**2,
                ...
                if col1 != col2:
                    ...
                    if col1 in [col for col in self.num_cols if col not in cat_cols]:
                        poly2_select.append(
                            (pl.col(col1).fill_nan(0).fill_null(0).pow(2))\
                                .alias(f"[{col1}]^2"))
                    else:
                        pass
            ...
            self.poly2_df.collect().write_parquet(f"{self.output_data_path}poly2_df.parquet")
    ...
    def AddFourier1(self, return_:bool = False):
        if self.transformations.fourier1:
            ...
            for col in fourier1_df.columns:
                fourier1_select.append((pl.col(col).fill_nan(0).fill_null(0)).sin().alias(f"sin([{col}])"))
                fourier1_select.append((pl.col(col).fill_nan(0).fill_null(0)).cos().alias(f"cos([{col}])"))
            fourier1_df = self.df.select(fourier1_select)

            # drop all non-inverse columns before hstacking
            self.fourier1_df = self._check_columns(df=fourier1_df, search_str=["sin", "cos"])

            # self.df = self.df.collect().hstack(fourier1_df.collect()).lazy()

            # save the fourier1_df to disk
            self.fourier1_df.collect().write_parquet(f"{self.output_data_path}fourier1_df.parquet")
    ...
    def AddFourier2(self, return_:bool = False):
        if self.transformations.fourier2:
            ...
            self.fourier2_df = self._check_columns(df=fourier2_df, search_str=["sin", "cos"])

            # self.df = self.df.collect().hstack(fourier2_df.collect()).lazy()

            # save the fourier2_df to disk
            self.fourier2_df.collect().write_parquet(f"{self.output_data_path}fourier2_df.parquet")
    ...
    def categorical_encoding(self,
        ...
        for col in df0.columns:
            if (col not in self.cat_cols) and \
                    (~pd.Series(df0.columns).str.lower().str.contains('dat')).tolist() and \
                    (~pd.Series(df0.columns).str.lower().str.contains('_dt')).tolist() and \
                    (~pd.Series(df0.columns).str.lower().str.contains('_id')).tolist() and \
                    (~pd.Series(df0.columns).str.lower().str.contains('_ind')).tolist() and \
                    (~pd.Series(df0.columns).str.lower().str.contains('missing')).tolist() and \
                        (df0.schema[col] != pl.Date) and \
                        (df0.schema[col] != pl.Datetime) and \
                        (df0.schema[col] != pl.Boolean) and \
                        (df0.schema[col] != pl.Categorical) and \
                        (df0.schema[col] != pl.Object) and \
                        (df0.schema[col] != pl.Utf8):
                ncol.append(col)
            elif ((~pd.Series(df0.columns).str.lower().str.contains('dat')).tolist() and \
                    (~pd.Series(df0.columns).str.lower().str.contains('_dt')).tolist() and \
                    (~pd.Series(df0.columns).str.lower().str.contains('_id')).tolist()) and (\
                        (df0.schema[col] == pl.Categorical) or \
                        (df0.schema[col] == pl.Object) or \
                        (df0.schema[col] == pl.Boolean) or \
                        (df0.schema[col] == pl.Utf8)):
                ccol.append(col)

        out  = aggregate_features(df = df0,
                        num_cols = ncol,
    ...
    def SumCategoricalEncoding(self, return_:bool = False):
        self.sum_cat_df = self.categorical_encoding(func = lambda x: x.sum(),
                                            func_name = "sum",
                                            include_relativities = True)

        self.sum_cat_df.collect().write_parquet(f"{self.output_data_path}sum_cat_df.parquet")

        if return_:
            return self.sum_cat_df

    def MeanCategoricalEncoding(self, return_:bool = False):
        self.mean_cat_df = self.categorical_encoding(func = lambda x: x.mean(),
                                            func_name = "mean",
                                            include_relativities = True)

        self.mean_cat_df.collect().write_parquet(f"{self.output_data_path}mean_cat_df.parquet")

        if return_:
            return self.mean_cat_df

    def MedianCategoricalEncoding(self, return_:bool = False):
        self.median_cat_df = self.categorical_encoding(func = lambda x: x.median(),
                                            func_name = "median",
                                            include_relativities = True)

        self.median_cat_df.collect().write_parquet(f"{self.output_data_path}median_cat_df.parquet")

        if return_:
            return self.median_cat_df

    def MinCategoricalEncoding(self, return_:bool = False):
        self.min_cat_df = self.categorical_encoding(func = lambda x: x.min(),
                                            func_name = "min",
                                            include_relativities = True)

        self.min_cat_df.collect().write_parquet(f"{self.output_data_path}min_cat_df.parquet")

        if return_:
            return self.min_cat_df

    def MaxCategoricalEncoding(self, return_:bool = False):
        self.max_cat_df = self.categorical_encoding(func = lambda x: x.max(),
                                            func_name = "max",
                                            include_relativities = True)

        self.max_cat_df.collect().write_parquet(f"{self.output_data_path}max_cat_df.parquet")

        if return_:
            return self.max_cat_df

    def StdCategoricalEncoding(self, return_:bool = False):
        self.std_cat_df = self.categorical_encoding(func = lambda x: x.std(),
                                            func_name = "std",
                                            include_relativities = True)

        self.std_cat_df.collect().write_parquet(f"{self.output_data_path}std_cat_df.parquet")

        if return_:
            return self.std_cat_df

    def VarCategoricalEncoding(self, return_:bool = False):
        self.var_cat_df = self.categorical_encoding(func = lambda x: x.var(),
                                            func_name = "var",
                                            include_relativities = True)

        self.var_cat_df.collect().write_parquet(f"{self.output_data_path}var_cat_df.parquet")

        if return_:
            return self.var_cat_df

    def SkewCategoricalEncoding(self, return_:bool = False):
        self.skew_cat_df = self.categorical_encoding(func = lambda x: x.skew(),
                                            func_name = "skew",
                                            include_relativities = True)

        self.skew_cat_df.collect().write_parquet(f"{self.output_data_path}skew_cat_df.parquet")

        if return_:
            return self.skew_cat_df

    def KurtosisCategoricalEncoding(self, return_:bool = False):
        self.kurtosis_cat_df = self.categorical_encoding(func = lambda x: x.kurtosis(),
                                            func_name = "kurtosis",
                                            include_relativities = True)

        self.kurtosis_cat_df.collect().write_parquet(f"{self.output_data_path}kurtosis_cat_df.parquet")

        if return_:
            return self.kurtosis_cat_df

    def IQRCategoricalEncoding(self, return_:bool = False):
        self.iqr_cat_df = self.categorical_encoding(func = lambda x: x.quantile(0.75) - x.quantile(0.25),
                                            func_name = "iqr",
                                            include_relativities = True)

        self.iqr_cat_df.collect().write_parquet(f"{self.output_data_path}iqr_cat_df.parquet")

        if return_:
            return self.iqr_cat_df

    def HarmonicMeanCategoricalEncoding(self, return_:bool = False):
        self.harmonic_mean_cat_df = self.categorical_encoding(func = lambda x: 1 / x.pow(-1).mean(),
                                            func_name = "harmonic_mean",
                                            include_relativities = True)

        self.harmonic_mean_cat_df.collect().write_parquet(f"{self.output_data_path}harmonic_mean_cat_df.parquet")

        if return_:
            return self.harmonic_mean_cat_df

    def AddCategoricalEncoding(self):
        encodings = [self.SumCategoricalEncoding,
