import pytest
from predictables.core.src._UnivariateAnalysis import _col_name_for_report
...
def test_col_name_for_report(input_col_name, expected_output):
    assert _col_name_for_report(input_col_name) == expected_output, (
        f"Expected '{expected_output}' for input '{input_col_name}', but got "
        f"'{_col_name_for_report(input_col_name)}'"
    )
...
def test_col_name_for_report_invalid(invalid_value):
    with pytest.raises(ValueError) as e:
        _col_name_for_report(invalid_value)
