...
class ExcelEDA:
    def __init__(
        ...
        if self.run_checks:
            ...
            assert isinstance(
                eda_df, (pl.DataFrame, pl.LazyFrame, pd.DataFrame)
            ), f"eda_df must be a polars or pandas DataFrame (actual: {type(eda_df)})"
            assert isinstance(
                feature_selection_df, (pl.DataFrame, pl.LazyFrame, pd.DataFrame)
...
(actual: {type(feature_selection_df)})"
            ...
            assert isinstance(
                wb, (openpyxl.Workbook, type(None))
        ...
        if self.run_checks:
            assert (
                self.feature_col_name in self.eda_df.columns
            ), f"eda_df must contain a column named {self.feature_col_name}.\
Actual columns:\n{self.eda_df.columns}"
            assert isinstance(
                self.eda_df, pl.LazyFrame
...
(actual: {type(self.feature_selection_df)})"
    ...
    def _get_image_files(self) -> list:
        ...
        if self.run_checks:
            ...
            assert (
                len(os.listdir(self.image_folder)) > 0
            ), f"image_folder {self.image_folder} must contain at least one image"
            assert any(
                [f.endswith(".png") for f in os.listdir(self.image_folder)]
    ...
    def _get_image_path(self, feature: str) -> str:
        ...
        if self.run_checks:
            ...
            assert (
                feature in self._get_feature_names()
            ), f"feature {feature} not found in image_folder {self.image_folder}\n\n\
Available features:\n{self._get_feature_names()}"
            assert os.path.isfile(
                os.path.join(self.image_folder, feature + ".png")
...
{self.image_folder}. Only .png files are supported."
    ...
    def _get_feature_names(self) -> list:
        ...
        if self.run_checks:
            # Validate inputs
            assert (
                len(self._get_image_files()) > 0
            ), f"image_folder {self.image_folder} must contain at least one image"
            assert any(
                [f.endswith(".png") for f in self._get_image_files()]
            ), f"image_folder {self.image_folder} must contain at least one .png file"
...
Available features:\n{self._get_feature_names()}"
            assert (
                feature in df[self.feature_col_name].to_list()
            ), f"feature {feature} not found in df\n\n\
Available features:\n{df[self.feature_col_name].to_list()}"

        feature_df = df.select([feature]).with_columns(
            [
                pl.divide(
...
{feature_df['normalized_stat'].max()})"
    ...
    def _normalize_feature_selection_df(self, df: pl.LazyFrame) -> pl.LazyFrame:
        ...
        if self.run_checks:
            ...
            assert (
                self.feature_col_name in df.columns
            ), f"df must contain a column named {self.feature_col_name}\
Actual columns:\n{df.columns}"

        normalized_df = df.select("feature_name").with_columns(
            [
...
{[f for f in self._get_feature_names() if f not in sorted_features]}"
    ...
    def _create_excel_workbook(self) -> None:
        ...
        if self.run_checks:
            # Validate inputs
            assert isinstance(
                self.wb, (openpyxl.Workbook, type(None))
...
Available features:\n{self._get_feature_names()}"
        ...
        stats = self.eda_df.filter(self.eda_df["feature_name"] == feature).to_dict(
            orient="records"
        )[0]
...
{i + self.stats_start_cell[1:]}"
        ...
        fs_stats = self.feature_selection_df.filter(
            self.feature_selection_df["feature_name"] == feature
...
{i + self.stats_start_cell[1:]}"
    ...
    def _save_chunks_to_files(self):
        ...
        chunks = [
            all_features[i : i + self.new_file_every_n]
