...
class FeatureSelection:
    def __init__(
        ...
        self.X_train, self.X_val, self.y_train, self.y_val = train_test_split(
            self.X_train, self.y_train, test_size=0.25, shuffle=True, random_state=42
        )
    ...
    def _logistic_fine_tune(self):
        ...
        logistic_search = GridSearchCV(
            LogisticRegression(
                penalty="l1", n_jobs=-1, fit_intercept=True, solver="saga", tol=0.0001
            ),
            self.logistic_grid,
            verbose=1,
        )
    ...
    def lasso_feature_selection(self, lambda_range=None, save=None):
        ...
        lambda_values = pd.DataFrame(
            [-1] * self.X_train.shape[1], columns=self.X_train.columns
        )
    ...
    def _rf_fine_tune(self):
        ...
        rf_search = GridSearchCV(
            RandomForestClassifier(), self.rf_grid, n_jobs=-1, verbose=1
        )
        rf_search.fit(X, y)
    ...
    def rf_feature_selection(self, save=None):
        ...
        rf_feature_importances = pd.DataFrame(
            importances, columns=["importance"], index=features
        )
        rf_feature_importances = rf_feature_importances.sort_values(
    ...
    def xgb_feature_selection(self, save=None):
        ...
        xgb_feature_importances = xgb_feature_importances.sort_values(
            by="importance", ascending=False
        )
    ...
    def _lgbm_fine_tune(self):
        ...
        lgbm_search = GridSearchCV(
            LGBMClassifier(), self.lgbm_grid, n_jobs=-1, verbose=1
        )
        lgbm_search.fit(X, y)
    ...
    def lgbm_feature_selection(self, save=None):
        ...
        lgbm_feature_importances = lgbm_feature_importances.sort_values(
            by="importance", ascending=False
        )
    ...
    def _cat_fine_tune(self):
        ...
        cat_search = GridSearchCV(
            CatBoostClassifier(), self.cat_grid, n_jobs=-1, verbose=1
        )
        cat_search.fit(X, y)
    ...
    def cat_feature_selection(self, save=None):
        ...
        cat_feature_importances = cat_feature_importances.sort_values(
            by="importance", ascending=False
        )
    ...
    def select_k_best(self, score_func, k=10):
        ...
        scores_df = pd.DataFrame(scores, columns=["score"], index=features)
