def get_significance_band(p_value: float, statistic: str) -> str:
    ...
    if p_value < 0.01:
        significance_statement = f"Extremely likely that {statistic} is significant"
    elif p_value < 0.05:
        significance_statement = f"Very likely that {statistic} is significant"
    elif p_value < 0.10:
        significance_statement = f"Somewhat likely that {statistic} is significant"
    else:
        significance_statement = f"Unlikely that {statistic} is significant"
