def NON_NUMERIC_COLUMNS():
    out = [  # BIN numbers are not categories and not useful for modeling
        "experian_bin",
        "cinergy_bin",  # these columns are not useful for modeling
        "cinergy_policy_numb",
        "cinergy_policy_module",
        "client_id",
        "evolve_eppid",
        # if I manage to actually include 5-digit zip code, maybe I'll use this
        "acct_prior_carrier_company_id",  # (but I doubt it at this point)
        "zip_extension",  # these columns are categorical integral codes
        ...
        "account_naics_3",
        "account_naics_4",
        "account_naics_5",  # these columns are categorical codes
        "agy_numb",  # these columns are repeated in the data
        "policy_prem",
        "cinergy_quoted_prem",
        "evolve_prem"  # this one assumes what I am trying to predict -- there is only issued
...
def NON_CATEGORICAL_COLUMNS():
    out = [  # already include the codes for the following columns, don't need a description
        ...
        "account_naics_5_desc",  # these are the same as the naics2 and naics3 descriptions
        "naics_sector",
        "naics_description",  # same idea, but for class code:
        "account_class_desc",
        "bop_primary_class_code_desc",
        "acct_bop_class_desc",  # not sure about this one
        # policy symbols only really tell me the product -- I am already assuming that
        "submission_id",  # the product is BOP
        "sub_policy_sym",
        "cinergy_policy_sym",
        # unclear whether or not I should include these, or whether they will be
        "full_policy_number",  # known when the model is used
        ...
        "agy_name_numb"  # I am not sure how to measure that at this point
