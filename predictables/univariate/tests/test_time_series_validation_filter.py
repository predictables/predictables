...
# )
# from predictables.util import filter_df_by_cv_fold, to_pd_df
...
#     )
#     train = filter_df_by_cv_fold(df, 2, to_pd_df(df)["cv2"], False, "train", "pd")
#     test = filter_df_by_cv_fold(df, 2, to_pd_df(df)["cv2"], False, "test", "pd")

#     expected_train_feature = train[["feature_col"]]
#     expected_train_target = train["target_col"]
#     expected_test_feature = test[["feature_col"]]
#     expected_test_target = test["target_col"]
#     (
#         assert_frame_equal(
...
#     (
#         assert_series_equal(
#             expected_train_target.reset_index(drop=True),
#             result[1].reset_index(drop=True),
#             f"y_train: {expected_train_target} is not the same as the one returned: {result[1]}",
#         )
#     )
#     (
#         assert_frame_equal(
#             (
#                 expected_test_feature.reset_index(drop=True)
#                 if isinstance(test, pd.DataFrame)
#                 else test.select(["feature_col"]).to_pandas()
#             ),
#             result[2].reset_index(drop=True),
#             f"X_test: {expected_test_feature} is not the same as the one returned: {result[2]}",
#         )
#     )
#     (
#         assert_series_equal(
#             expected_test_target.reset_index(drop=True),
#             result[3].reset_index(drop=True),
#             f"y_test: {expected_test_target} is not the same as the one returned: {result[3]}",
#         )
#     )
