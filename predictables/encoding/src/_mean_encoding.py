...
def mean_encoding_with_ratio_lazy(
    ...
    laplace_label = (
        f"(laplace_alpha={laplace_alpha})" if laplace_alpha > 0 else ""
    )
    ...
    if pl.__version__ >= "0.20.5":
        lazy_df = df.lazy().with_columns(
            pl.arange(0, pl.len()).alias("row_ord")
        )
    else:
        lazy_df = df.lazy().with_columns(
            pl.arange(0, pl.count()).alias("row_ord")
        )
        ...
        .with_columns(
            [
                (
                    ...
                    / (
                        pl.col("sum_col2").shift(date_offset)
