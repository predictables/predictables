        ...
        .with_columns(
            [
                pl.cum_sum_horizontal(
                    means.select(cs.contains("boot_mean")).columns
                ).alias("cum_sum")
            ]
        )
        .with_columns([(pl.col("cum_sum") / n_boot).alias("cum_mean")])
        .with_columns(
            [
                pl.reduce(
                    function=lambda acc, x: acc + (x - pl.col("cum_mean")) ** 2,
                    exprs=cs.contains("boot_mean"),
                    init=0,
                )
                .truediv(n_boot - 1)
                .alias("variance")
