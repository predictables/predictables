...
def calculate_probabilities(base_prob, levels=4, seed=None):
    if levels <= 1:
        raise ValueError("The number of levels must be greater than one to generate noise properly.")
    
    np.random.seed(seed)
    logits = np.log(max(min(base_prob, 1 - 1e-10), 1e-10) / (1 - max(min(base_prob, 1 - 1e-10), 1e-10)))
    sd = np.sqrt(np.abs(logits) * 10)
    noises = np.random.normal(0, sd, levels - 1)
...
def generate_full_naics_data(seed=42):
    base_probs = {f"{10+i}": p for i, p in enumerate([0.05, 0.15, 0.25, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85, 0.95])}
    data = []
...
def sample_and_set_targets(data, n=10000, seed=42):
    sampled_data = data.sample(n=n, replace=True, weights='probability', random_state=seed)
    sampled_data['target'] = sampled_data['probability'].apply(lambda p: np.random.binomial(1, p))
    return sampled_data

# Validation functions
def validate_data(data):
    # Check hierarchical structure
    structure_check = all(data['naics_3_cd'].str.startswith(data['naics_2_cd']) &
...
if __name__ == "__main__":
    ...
    print("Hierarchical Structure Valid:", structure_valid)
