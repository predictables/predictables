...
def main_function(dataframe, target_variable, interaction_variable, hover_data_variables, shap_values, filters=None, interactive_features=True):
    """Main function to create an interactive SHAP summary plot."""
    validate_dataframe(dataframe)
    validate_column_presence(dataframe, [target_variable, interaction_variable] + hover_data_variables)
    validate_shap_values(shap_values)
    prepared_data = extract_and_filter_data(dataframe, target_variable, interaction_variable, hover_data_variables, filters)
...
def validate_column_presence(dataframe, columns):
    ...
    if missing_columns:
        raise ValueError("The following columns are missing from the dataframe: {}".format(', '.join(missing_columns)))
...
def validate_shap_values(shap_values):
    ...
    if not isinstance(shap_values, (np.ndarray, pd.DataFrame)):
        raise ValueError("SHAP values must be provided as a numpy array or a pandas DataFrame.")

def extract_and_filter_data(dataframe, target_variable, interaction_variable, hover_data_variables, filters):
    """Extract and optionally filter data based on user-defined conditions."""
    relevant_data = dataframe[[target_variable, interaction_variable] + hover_data_variables]
    if filters:
...
def create_base_plot(data, shap_data, interactive_features):
    ...
    plot = px.scatter(data, x=shap_data, y=data[target_variable], color=shap_data,
                      title="SHAP Summary Plot", hover_data=data.columns)
...
if __name__ == "__main__":
    ...
    # Generate the plot
    plot = main_function(df, 'target', 'interaction', ['feature1', 'feature2'], shap_vals, filters)
    plot.show()